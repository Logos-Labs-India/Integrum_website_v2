...
/* ---------- India map — correct external boundary, embedded (india-geo.js) ---------- */
...
const PROJECT_REGIONS = [
  ...
  { name: "Tamil Nadu",  lng: 78.3, lat: 11.0, status: "development", color: "#1D9E75", glow: "#C084FC", off: [[10,-9],[-8,11]] },
];
    ...
    <div className="map-wrap" style={{height}}>
      ...
      {hover && (()=>{ const r=pins.find(x=>x.name===hover);
        return <div className="map-tooltip" style={{left:`${(r.x/INDIA_W)*100}%`, top:`${(r.y/INDIA_H)*100}%`}}>
          <div className="mt-t">{r.name}</div><div className="mt-m num">{r.status==="development" ? "Under development" : `${r.n} projects`}</div>
        </div>; })()}
    </div>
  );
}
    ...
    <div className="map-wrap" style={{ height }}>
      <svg viewBox={INDIA_VB} preserveAspectRatio="xMidYMid meet" style={{ width: "100%", height: "100%", overflow: "visible" }}>
        <defs>
          ...
          {regions.map((rg, i) => (<radialGradient id={`halo${i}`} key={i}><stop offset="0" stopColor={rg.glow} stopOpacity="0.40"/><stop offset="100%" stopColor={rg.glow} stopOpacity="0"/></radialGradient>))}
          <linearGradient id="indiaFill" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0" stopColor="#6FB0DE" stopOpacity="0.16"/>
            <stop offset="100%" stopColor="#6FB0DE" stopOpacity="0.05"/>
          </linearGradient>
        </defs>
        {/* soft halos read through the translucent landmass */}
        {regions.map((rg, i) => (
          <circle key={"h"+i} cx={rg.x} cy={rg.y} r={hover === rg.name ? 56 : 44} fill={`url(#halo${i})`} style={{ transition: "r .3s ease" }}/>
        ))}
        {/* the outline itself — one continuous stroke, as in the reference */}
        {geo && (
          <path d={geo.d} fill="url(#indiaFill)" stroke="rgba(190,222,247,.92)"
            strokeWidth="1.15" strokeLinejoin="round" strokeLinecap="round" vectorEffect="non-scaling-stroke"/>
        )}
        {regions.map((rg) => {
          ...
          return (
            <g key={rg.name+"p"} className="map-pin" onMouseEnter={() => setHover(rg.name)} onMouseLeave={() => setHover(null)} style={{ cursor: "pointer" }}>
              {rg.off.map((o,j)=>(
                <circle key={j} cx={rg.x+o[0]} cy={rg.y+o[1]} r="3" fill={rg.glow} opacity="0.55"/>
              ))}
              <circle cx={rg.x} cy={rg.y} r={on ? 9 : 7.5}
                fill={on ? rg.glow : "rgba(255,255,255,.96)"} stroke={rg.glow} strokeWidth={on ? 0 : 2}
                filter="url(#pinglow)" style={{ transition:"fill .2s ease, r .2s ease" }}/>
            </g>
          );
        })}
      </svg>
      {hover && (() => { const r = regions.find(x => x.name === hover);
        return <div className="map-tooltip" style={{ left: `${(r.x/INDIA_W)*100}%`, top: `${(r.y/INDIA_H)*100}%` }}><div className="mt-t">{r.name}</div><div className="mt-m num">{r.status==="development" ? "Under development" : `${r.n} projects`}</div></div>; })()}
    </div>
  );
}
