...
function buildSearchIndex() {
  const base = [
    ...
    { label:"Reduce your Energy bill", desc:"Reduce your energy bill — hybrid, storage, EaaS", go:"cni", kind:"Solutions" },
...
function Footer({ nav }) {
  const cols = [
    { h:"Solutions", links:[["Reduce your energy bill","cni"],["Hybrid wind + solar","cni"],["Energy storage · BESS","cni"],["CAPEX · OPEX · EaaS","cni"]] },
...
function App() {
  ...
  const parsePath = () => {
    ...
    const raw = p.replace(/^\/+|\/+$/g, "").replace(/^index\.html?$/, "");
  ...
  const nav = (p) => {
    ...
    history.pushState(null, "", (window.APP_BASE || "/") + (top === "home" ? "" : path));
    if (hash) {
  ...
  useEffect(()=>{
    ...
    const base = "https://integrumenergy.in/";
    const fullPath = route.sub ? page + "/" + route.sub : page;
    const url = page === "home" ? base : base + fullPath;
    ...
    // BreadcrumbList for this route
    const crumbLabel = (NAV_LINKS.find(l=>l.page===page) || {}).label
      || page.charAt(0).toUpperCase() + page.slice(1);
    const crumbs = [{ "@type":"ListItem", position:1, name:"Home", item:base }];
    if (page !== "home") crumbs.push({ "@type":"ListItem", position:2, name:crumbLabel, item:url });
