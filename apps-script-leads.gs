...
function doPost(e) {
  ...
  try {
    ...
    if (NOTIFY_EMAIL) {
      try {
        // Build from the submitted data itself (not the sheet headers), and
        // skip empty fields, so the email always shows what was actually sent.
        var body = keys.filter(function (k) {
          return k !== 'resume_file' || String(data[k]).indexOf('http') === 0;
        }).map(function (k) {
          var v = data[k];
...
var ALIAS_ = {
  ...
  routedto:       'routeto',
  routedtoemail:  'routeto',
  resumesizebytes:'resumesize',
};
